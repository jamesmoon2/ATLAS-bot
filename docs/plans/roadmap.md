...
ATLAS is a personal AI assistant Discord bot wrapping Claude Code CLI or Codex, integrated with an Obsidian vault (second brain), Oura Ring, WHOOP, Garmin, Google Calendar, Gmail, and weather APIs. It currently has 15 cron jobs, 10 skills, repo-managed MCP servers, and a hook-based context injection system.
...
| `#atlas-dev` | ATLAS harness and operational alerts          | MCP health check, ops watchdog, session archive |
...
- `!help` is channel-aware, and `!status` / `!ops` exposes bot, service, cron, and MCP helper health.
